...
@Singleton
public class DeclarationProjectController {
  ...
  public void generateProject(ActionRequest request, ActionResponse response) {
    try {
      ...
      if (projectGeneratorType.equals(ProjectGeneratorType.PROJECT_ALONE)) {
        project = factory.create(declaration);
      } else {
        project = factory.generate(declaration, startDate);
      }
    ...
    try {
      ...
      if (declaration.getDeclarationLineList() == null
          || (declaration.getDeclarationLineList() != null
              && declaration.getDeclarationLineList().isEmpty())) {
        response.setAlert(
            I18n.get(BusinessProjectExceptionMessage.SALE_ORDER_GENERATE_FILL_PROJECT_ERROR_2));
        return;
      }
      ...
      ActionViewBuilder view = factory.fill(declaration.getProject(), declaration, startDate);
  ...
  public void updateLines(ActionRequest request, ActionResponse response) {
    try {
      Declaration declaration = request.getContext().asType(Declaration.class);
      declaration = Beans.get(DeclarationRepository.class).find(declaration.getId());
      declaration = Beans.get(ProjectAnalyticMoveLineService.class).updateLines(declaration);
