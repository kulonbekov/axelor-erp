 ...
 */
package com.axelor.apps.sale.service.declaration.pricing;
...
public class DeclarationLinePricingObserver implements PricingObserver {

  private final DeclarationLine declarationLine;
  private StringBuilder logs;

  public DeclarationLinePricingObserver(DeclarationLine declarationLine) {
    this.declarationLine = Objects.requireNonNull(declarationLine);
  }
  ...
  public void computationFinished() {
    if (logs.length() == 0) {
      this.declarationLine.setPricingScaleLogs(
          I18n.get(ITranslation.SALE_ORDER_LINE_OBSERVER_NO_PRICING));
    } else {
      this.declarationLine.setPricingScaleLogs(logs.toString());
    }
  }
