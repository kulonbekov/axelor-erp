/*
 * Axelor Business Solutions
 *
 * Copyright (C) 2005-2023 Axelor (<http://axelor.com>).
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */
package com.axelor.apps.supplychain.service;

import com.axelor.apps.base.AxelorException;
import com.axelor.apps.base.db.Company;
import com.axelor.apps.sale.db.Declaration;
import com.axelor.apps.sale.db.DeclarationLine;
import com.axelor.apps.stock.db.StockMove;
import com.axelor.apps.stock.db.StockMoveLine;
import java.math.BigDecimal;
import java.time.LocalDate;
import java.util.List;
import java.util.Optional;

public interface DeclarationStockService {

  /**
   * Create a delivery stock move from a sale order.
   *
   * @param declaration
   * @return
   * @throws AxelorException
   */
  public List<Long> createStocksMovesFromDeclaration(Declaration declaration) throws AxelorException;

  public StockMove createStockMove(
      Declaration declaration, Company company, LocalDate estimatedDeliveryDate) throws AxelorException;

  public StockMoveLine createStockMoveLine(StockMove stockMove, DeclarationLine declarationLine)
      throws AxelorException;

  public StockMoveLine createStockMoveLine(
      StockMove stockMove, DeclarationLine declarationLine, BigDecimal qty) throws AxelorException;

  public boolean isStockMoveProduct(DeclarationLine declarationLine) throws AxelorException;

  boolean isStockMoveProduct(DeclarationLine declarationLine, Declaration declaration)
      throws AxelorException;

  /**
   * Update delivery state by checking delivery states on the sale order lines.
   *
   * @param declaration
   */
  void updateDeliveryState(Declaration declaration) throws AxelorException;

  /**
   * Update delivery states in sale order and sale order lines.
   *
   * @param declaration
   * @throws AxelorException
   */
  void fullyUpdateDeliveryState(Declaration declaration) throws AxelorException;

  /**
   * Find the sale order linked to the stock move.
   *
   * @param stockMove a stock move
   * @return the found sale order, or an empty optional if no sale order was found.
   */
  Optional<Declaration> findDeclaration(StockMove stockMove);
}
