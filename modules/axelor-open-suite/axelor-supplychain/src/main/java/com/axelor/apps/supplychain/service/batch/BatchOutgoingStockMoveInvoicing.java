...
public class BatchOutgoingStockMoveInvoicing extends BatchStrategy {
  ...
  protected void process() {
    ...
    DeclarationRepository saleRepo = Beans.get(DeclarationRepository.class);
        ...
        JPA.em()
            .createQuery(
                "SELECT self FROM StockMove self "
                    ...
                    + "AND self.originTypeSelect LIKE :typeDeclaration "
            ...
            .setParameter("typeDeclaration", StockMoveRepository.ORIGIN_SALE_ORDER)
    ...
    while (!(stockMoveList = query.getResultList()).isEmpty()) {
      for (StockMove stockMove : stockMoveList) {
        try {
          stockMoveInvoiceService.createInvoiceFromDeclaration(
